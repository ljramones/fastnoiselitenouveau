package com.cognitivedynamics.noisegen.graph.nodes.combiner;

import com.cognitivedynamics.noisegen.graph.NoiseNode;

/**
 * Combiner node that multiplies two noise sources.
 *
 * <p>Output = A * B
 *
 * <p>This class is immutable and thread-safe.
 */
public final class MultiplyNode extends CombinerNode {

    /**
     * Create a multiply node.
     *
     * @param inputA The first input
     * @param inputB The second input
     */
    public MultiplyNode(NoiseNode inputA, NoiseNode inputB) {
        super(inputA, inputB);
    }

    @Override
    protected double combine(double a, double b) {
        return a * b;
    }

    @Override
    public String getNodeType() {
        return "Multiply";
    }

    @Override
    public String toString() {
        return "MultiplyNode(" + inputA.getNodeType() + " * " + inputB.getNodeType() + ")";
    }
}
